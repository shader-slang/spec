...
\CheckType{\ContextVarA}{\MetaVar{T}}{\ContextVarB} \\
\CheckDecl{\ContextVarA}{\code{typealias} \MetaVar{A} = \MetaVar{T}\code{;}}{\ContextVarA,\code{type}\MetaVar{A}\code{=}\MetaVar{T}}
...
Any conformance that does not follow these rules (that is, where both \Char{T} and \ChVar{I} are imported into module \Char{M}) is called a \SpecDef{retroactive} conformance, and there is no way to guarantee that another module \Char{N} will not introduce the same conformance.

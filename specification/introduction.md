...
* A  *meta-value* name may be rendered in italics and _UpperCamelCase_. E.g., _IfStatement_, _UnitType_
...
In each case, the concept being referenced is the same; for example, the references _UnitType_, `Unit`, and *unit type* all refer to the same definition.
...
A  *meta-variable* is rendered in italics, and must either be in _lowerCamelCase_, or consist of a single letter (either upper- or lower-case).
For example, text might introduce a  *meta-variable* named _e_ that represents an unknown  *expression*.

...
A variable declaration must have either a \SynRef{TypeAscription} or an \SynRef{InitialValue}; a variable declaration may have both.
...
If a variable declaration has a \SynRef{TypeAscription} and no \SynRef{InitialValue}, then the type of that variable declaration is \MetaVar{T}.

If a variable declaration has a \SynRef{TypeAscription} and a \SynRef{InitialValue}, then let \MetaVar(E) be the value that results from evaluating the expression of the initial value caluse

If a variable declaration has a \SynRef{InitialValue} but not \SynRef{TypeAscription}, then the

no \SynRef{TypeAscription}, then the type of the variable is determined by evaluating its type expression in the current environment.
If a variable declaration with a \SynRef{TypeAscription} also has an \SynRef{InitialValue}, then 
